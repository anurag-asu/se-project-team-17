...
def collect_dimensionality_metric():
    try:
        ...
        for link in links:
            ...
            found = df.loc[
                df["corrected_pj_github_url"] == link, "correct_url_found"
            ].bool()
            if not found:
            ...
            if os.path.basename(link) in repos:
                ...
                value = float(
                    df.loc[df["corrected_pj_github_url"] == link, "dimensionality"]
                )
                if value > 0.0:
                    continue
                ...
                df.loc[
                    df["corrected_pj_github_url"] == link, "dimensionality"
                ] = dimensionality
...
def get_pulls(repoLink):
    ...
    page_num = 1
    path = "https://api.github.com/repos{}/pulls?sort=created&direction=asc&page={}&per_page=100&state=all".format(
        url_obj.path, page_num
    )
    ...
    try:
        ...
        for link in links:
            ...
            try:
                ...
                created_start_date = datetime.strptime(
                    created_start_date, "%Y-%m-%dT%H:%M:%SZ"
                )
                created_end_date = datetime.strptime(
                    created_end_date, "%Y-%m-%dT%H:%M:%SZ"
                )

                start_date = datetime(
                    year=created_start_date.year,
                    month=created_start_date.month,
                    day=created_start_date.day,
                )
                end_date = datetime(
                    year=created_end_date.year,
                    month=created_end_date.month,
                    day=created_end_date.day,
                    hour=23,
                    minute=59,
                    second=59,
                )
                ...
                end_range = start_date + timedelta(
                    days=num_days, hours=23, minutes=59, seconds=59
                )
                ...
                while start_range <= end_date:
                    ...
                    for created_at in created_at_list:
                        ...
                        if (
                            created_at_date >= start_range
                            and created_at_date <= end_range
                        ):
                            count += 1

                    commits_per_interval.append(count)
                    start_range = datetime(
                        year=end_range.year, month=end_range.month, day=end_range.day
                    ) + timedelta(days=1)
                    end_range = start_range + timedelta(
                        days=num_days, hours=23, minutes=59, seconds=59
                    )

                # print('commits per interval', sum(commits_per_interval), commits_per_interval)
                avg_commits_per_range = sum(commits_per_interval) / len(
                    commits_per_interval
                )
                df.loc[
                    df["corrected_pj_github_url"] == link, "frequency"
                ] = avg_commits_per_range
                df.to_csv("csvs/asfi_refined_with_metrics.csv", index=False)
            except Exception as e:
                print(
                    "error collect_frequency_index for link = {} with erro = {}".format(
                        link, e
                    )
                )
