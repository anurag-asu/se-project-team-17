...
def pre_process_data_metric(df, metric):
    ...
    print('projects with ' + metric + ' higher than mean and graduated ', len(df_mean_high_gtd))
    print('projects with ' + metric + ' higher than mean and retired ', len(df_mean_high_rtd))

    df_mean_low_gtd = df_mean_low.loc[df['status'] == 1]
    df_mean_low_rtd = df_mean_low.loc[df['status'] == 2]

    print('projects with ' + metric + ' lower than mean and graduated ', len(df_mean_low_gtd))
    print('projects with ' + metric + ' lower than mean and retired ', len(df_mean_low_rtd))

    chi_sq(len(df_mean_high_gtd),len(df_mean_low_gtd), len(df_mean_high_rtd), len(df_mean_low_rtd))
...
def chi_sq(df_mean_high_gtd, df_mean_low_gtd, df_mean_high_rtd, df_mean_low_rtd):
    ...
    # contingency table
    table = [	
                [df_mean_high_gtd, df_mean_low_gtd],
                [df_mean_high_rtd, df_mean_low_rtd]
            ]
...
def pre_process_data(metric):
    df = pd.read_csv('csvs/asfi_refined_with_metrics.csv')
    df = df.loc[(df[metric] != NaN) & (df['correct_url_found'] != False) & (df['status'] != 0)]
...
def normalize_metrics(metric, show_graph = False):
    df = pd.read_csv('csvs/asfi_refined_with_metrics.csv')
    df = df.loc[(df[metric] != NaN) & (df['correct_url_found'] != False) & (df['status'] != 0)]
...
pre_process_data('frequency')
pre_process_data('dimensionality')
# pre_process_data_frequency()

# plot_graphs('frequency')
